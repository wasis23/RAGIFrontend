...
12. **Posisi Tombol Pengaturan Global / Toggle Status**: Tombol toggle pengaturan global atau status modul (seperti `Skema UKT: ON/OFF`, mode switcher modul, dsb.) WAJIB ditempatkan di `PageHeader action` paling atas, berdampingan di sebelah KIRI tombol Filter dengan urutan: `[Tombol Toggle / Setting Global] -> [Filter] -> [Tambah Data]`. DILARANG menaruh tombol toggle konfigurasi global di dalam tabel atau table action bar.
</RULE[admin_crud_reviewer]>
