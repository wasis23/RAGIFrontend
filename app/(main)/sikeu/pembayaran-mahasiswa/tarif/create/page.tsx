...
export default function CreateTarifBiayaPage() {
  ...
  useEffect(() => {
    const fetchReferences = async () => {
      setLoadingRefs(true);
      try {
        const [resKatalog, resProdi, resAngkatan, resTarifList] = await Promise.all([
        ...
        if (katalog.length > 0) {
          setValue('master_biaya_id', katalog[0].id);
          if (katalog[0].nominal_standar) {
            setValue('nominal', Number(katalog[0].nominal_standar));
          }
        }
      } catch (err) {
        console.error('Error fetching references:', err);
        toast.error('Gagal memuat data referensi katalog biaya & prodi');
      } finally {
  ...
  // Deteksi real-time apakah komponen biaya pada angkatan & semester ini sudah aktif untuk Semua Program Studi (Global)
  const semesterOverlap = (tSemester: any, formSemester: string) => {
    if (!formSemester) return true; // tarif baru semua semester: tabrakan dengan apa pun
    return tSemester === null || tSemester === undefined || Number(tSemester) === Number(formSemester);
  };

  const existingGlobalTarif = useMemo(() => {
    if (!watchMasterBiayaId || !watchAngkatan) return null;
  ...
  }, [watchMasterBiayaId, watchAngkatan, watchSemester, existingTarifs]);

  // Deteksi real-time apakah sudah ada tarif spesifik per prodi yang aktif untuk komponen, angkatan & semester ini
  const existingProdiTarifs = useMemo(() => {
    if (!watchMasterBiayaId || !watchAngkatan) return [];
  ...
  // Auto-fill nominal standar saat komponen biaya katalog dipilih
  ...
  const onSubmitForm = async (formData: TarifFormData) => {
    ...
    try {
      ...
      toast.success('Pengaturan tarif komponen biaya berhasil ditambahkan!');
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      <PageHeader
        title="Tambah Tarif Komponen Biaya"
        description="Penetapan tarif baru dari katalog komponen biaya berdasarkan program studi dan tahun angkatan mahasiswa."
        action={
          <Link
      ...
      {loadingRefs ? (
        <div className="p-12 bg-white border border-slate-200 rounded-2xl flex flex-col items-center justify-center text-slate-400 gap-3">
          ...
          <p className="text-xs font-semibold">Memuat master katalog biaya & opsi program studi...</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-6">
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3 shadow-2xs">
              ...
              <div>
                ...
                <p className="text-xs text-amber-800 mt-1">
                  Komponen biaya ini sudah disetting aktif berlaku menyeluruh untuk <strong>Semua Program Studi</strong> pada Angkatan {watchAngkatan} sebesar <strong>{formatRupiah(existingGlobalTarif.nominal)}</strong>.
                  Sesuai kebijakan satu pintu, Anda tidak perlu dan tidak diperbolehkan menginputkan tarif lagi untuk angkatan ini.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-2xs space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {/* Field 7: Keterangan / Dasar Kebijakan (Rentang Penuh) */}
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              >
                {submitting ? 'Menyimpan Tarif...' : 'Simpan Pengaturan Tarif'}
              </Button>
            </div>
          </div>
        </form>
