...
export default function EditAsetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const asetId = parseInt(resolvedParams.id);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      setIsLoading(true);
      try {
        const res = await sinapraService.getAsetDetail(asetId);
        const data = res.data;
        if (data) {
          if (data.kategori) {
            setSelectedKategoriObj({ value: data.kategori.id.toString(), label: `${data.kategori.kode} - ${data.kategori.nama}` });
          }
          if (data.ruangan) {
        ...
        toast.error('Gagal memuat detail barang aset.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sinapraService.updateAset(asetId, formData);
      toast.success('Data barang inventaris aset berhasil diperbarui!');
      router.push('/sinapra/aset');
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Gagal memperbarui barang aset.');
    } finally {
