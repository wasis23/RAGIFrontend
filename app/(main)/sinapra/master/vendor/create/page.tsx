...
export default function CreateMasterVendorPage() {
    ...
    const loadJenisRekanan = async () => {
      try {
        ...
        if (Array.isArray(res) && res.length > 0) {
          ...
          setJenisRekananOptions(opts);
          setValue('jenis_rekanan', opts[0].value);
        }
      } catch {
        // Fallback silently
      }
  ...
  const onSubmit = async (data: VendorFormData) => {
    try {
      await sinapraService.createMasterVendor(data);
      toast.success('Vendor baru berhasil ditambahkan');
      router.push('/sinapra/master/vendor');
    } catch (err: any) {
      toast.error(err?.response?.data?.message || 'Gagal menambahkan vendor');
    }
  };
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <PageHeader
        ...
        description="Pendaftaran data rekanan pengadaan barang, jasa pemeliharaan sarana, atau laboratorium kalibrasi"
        action={
          <Button
            variant="outline"
